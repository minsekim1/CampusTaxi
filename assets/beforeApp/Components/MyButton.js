...
//<MyButton title="버튼텍스트" color='red' type="" icon="arrow-left" iconSize={} onPress={} />

//p => props
function MyButton(p) {
  const color = p.color == null ? "white" : p.color;
  const backgroundColor =
    p.backgroundColor == null ? "#0D3664" : p.backgroundColor;
  ...
  if (
    ...
    return (
      <Button
        titleStyle={styles.containerViewStyleDefault}
        containerStyle={styles.containerStyleDefault}
        title={p.title}
        type={p.type}
        buttonStyle={styles.buttonStyle}
        onPress={p.onPress}
        icon={<Icon name={p.icon} size={iconSize} color={iconColor}></Icon>}
      ></Button>
    );
  } else {
    return <Button title="No Type !" style={styles.default}></Button>;
  }
}
